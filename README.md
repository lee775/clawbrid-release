...
- **Image Generation** - 로컬 Stable Diffusion WebUI 연동 이미지 생성/합성/업스케일
...
- **Stable Diffusion WebUI** - 이미지 생성/합성 ([Automatic1111](https://github.com/AUTOMATIC1111/stable-diffusion-webui), `--api` 플래그 필요)
...
### clawbrid-image
Claude Code에서 이미지 생성/합성이 필요할 때 자동으로 호출됩니다. 로컬 Stable Diffusion WebUI (`--api` 모드) 필요.
...
| `image_models` | 설치된 모델 목록 조회/변경 |
| `image_status` | SD WebUI 연결 상태 확인 |

**설정** (선택): `~/.clawbrid/config.json`에 SD WebUI URL 지정
```json
{ "stableDiffusion": { "url": "http://127.0.0.1:7860" } }
```
...
Claude Code CLI ──── MCP: clawbrid-cron   (stdio)
                ...
                └─── MCP: clawbrid-image  (stdio) → SD WebUI API
```
