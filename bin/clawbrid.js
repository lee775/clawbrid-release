...
`);
  ...
  update() {
    ...
    // 1. 파일 잠금 해제: Tauri 모니터 종료 + PM2 clawbrid 프로세스 완전 삭제
    ...
    // 프로세스가 파일 핸들 놓을 때까지 대기
    execSync('ping 127.0.0.1 -n 3 >nul', { stdio: 'ignore', windowsHide: true });

    // 2. npm install
    console.log('  Checking for updates...\n');
    try {
      execSync('npm install -g lee775/clawbrid-release', { stdio: 'inherit', windowsHide: true });
      console.log('\n  Update complete!');
    } catch (err) {
      console.error(`\n  Update failed: ${err.message}`);
    }

    // 3. PM2 다시 등록 (delete했으므로 start로 새로 등록)
    const newRoot = path.join(CLAWBRID_ROOT);
    try { execSync(`pm2 start "${path.join(newRoot, 'src', 'bridges', 'slack-standalone.js')}" --name clawbrid-slack`, { stdio: 'ignore', windowsHide: true }); } catch {}
    try { execSync(`pm2 start "${path.join(newRoot, 'src', 'bridges', 'telegram-standalone.js')}" --name clawbrid-telegram`, { stdio: 'ignore', windowsHide: true }); } catch {}
    try { execSync(`pm2 start "${path.join(newRoot, 'src', 'cron-worker.js')}" --name clawbrid-cron`, { stdio: 'ignore', windowsHide: true }); } catch {}
