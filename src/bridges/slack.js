...
const QA_LOG_FILE = path.join(config.CONFIG_DIR, 'slacklog.txt');
    ...
    if (promptStructurer.shouldStructure(text)) {
      try {
        ...
        if (structured && structured !== text) {
          console.log(`[SLACK] 질문 구조화 | ${text.length}자 → ${structured.length}자`);
          prompt = structured;
        }
      } catch (e) { console.error(`[SLACK] 질문 구조화 실패: ${e.message}`); }
    }
