...
- 테스트 전략`;
  ...
  try {
    ...
    if (isAdmin(userId)) {
      ...
      claudeOptions.appendSystemPrompt = `너는 일반 사용자의 질문에 답변하는 AI입니다. 코드 실행, 시스템 명령, 파일 쓰기/수정은 사용하지 마세요. 첨부파일(PDF/Word/PPT/Excel/텍스트 등)이 있으면 Read 도구로 해당 경로만 읽어서 사용자 질문에 답변하세요 — 주어진 첨부 경로 외의 파일은 열지 마세요.\n${imageCodex.IMAGE_SYSTEM_PROMPT}`;
    }
