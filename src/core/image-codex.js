...
Prompt: ${englishPrompt}`;
  ...
  // 2. Codex 기본 경로(~/.codex/generated_images)도 스캔. Codex는 같은 이미지를
  //    IMAGE_DIR과 기본 경로 양쪽에 저장하는 경우가 있어, IMAGE_DIR에 이미 새 파일이 있으면
  //    기본 경로의 새 파일은 중복으로 간주하고 삭제. 비어 있을 때만 IMAGE_DIR로 이동.
  const afterCodex = snapshotImages(CODEX_IMAGE_DIR);
  const newCodex = [...afterCodex].filter((f) => !beforeCodex.has(f));
  const movedFromCodex = [];
  if (newLocal.length) {
    // IMAGE_DIR에 이미 저장된 게 있으므로 기본 경로 사본은 폐기 (중복 전송 방지)
